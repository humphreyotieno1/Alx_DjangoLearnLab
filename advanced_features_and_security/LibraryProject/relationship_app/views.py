...
from .forms import BookForm, LibraryForm, UserRegisterForm
...
def is_librarian(user):
    return hasattr(user, 'role') and user.role == 'Librarian'

def is_member(user):
    return hasattr(user, 'role') and user.role == 'Member'
...
def book_list(request):
    ...
    if not request.user.has_perm('relationship_app.can_view_book'):
    ...
    return render(request, 'relationship_app/book_list.html', {
        ...
        'can_add_book': request.user.has_perm('relationship_app.can_create_book'),
        'can_edit_book': request.user.has_perm('relationship_app.can_edit_book'),
...
def edit_book(request, pk):
    ...
    if not request.user.has_perm('relationship_app.can_edit_book'):
...
def bulk_delete_books(request):
    try:
        book_ids = json.loads(request.body).get('book_ids', [])
        if not book_ids:
        ...
        if not request.user.has_perm('relationship_app.can_delete_book'):
            return JsonResponse(
                {'status': 'error', 'message': 'You do not have permission to delete books'}, 
                status=403
            )
        ...
        return JsonResponse({
            'status': 'success',
            'message': f'Successfully deleted {len(book_ids)} book(s)'
        })
    except Exception as e:
